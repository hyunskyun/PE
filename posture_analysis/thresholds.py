...
여기 있는 숫자들은 일반적인 스프린트 생체역학 자료를 참고한 '예시 기준값'
이다. 실제 탐구에서는 코치 자문이나 논문 수치로 교체해 쓰는 것을 권장한다.
구조를 단순한 dict로 유지한 이유는, 학생이 숫자만 바꿔도 판정 기준을
쉽게 조정할 수 있게 하기 위함이다.
"""
...
THRESHOLDS = {
    ...
    MAX_VELOCITY: {
        ...
        "stride_ratio": (0.95, 1.25),
        "landing_offset_ratio": (-0.05, 0.08),
    },
}

# metric, 방향("low"/"high") 조합별 오류 이름과 교정 문장
CORRECTION_MESSAGES = {
    ...
    ("trunk_lean_deg", "low", MAX_VELOCITY): (
        "상체 후경(뒤로 젖힘) 의심",
        "최대 속도 구간에서 상체가 너무 세워져 있습니다. 살짝 앞으로 기울이세요.",
    ),
    ...
    ("knee_angle_deg", "low", ACCELERATION): (
        "무릎 굴곡 과다",
        "무릎이 너무 접혀 있습니다. 지면을 밀어내는 힘을 더 크게 써 보세요.",
    ),
    ("knee_angle_deg", "high", ACCELERATION): (
        "무릎 신전 과다(가속 구간)",
        "가속 구간에서 다리가 너무 펴져 있습니다. 무릎을 더 끌어올리세요.",
    ),
    ...
    ("knee_angle_deg", "high", MAX_VELOCITY): (
        "무릎 과신전 의심",
        "무릎이 과도하게 펴져 있습니다. 착지 충격 흡수를 확인하세요.",
    ),
    ...
    ("stride_ratio", "low", ACCELERATION): (
        "보폭 부족(가속 구간)",
        "보폭이 좁습니다. 지면을 뒤로 밀어내는 힘을 더 키워보세요.",
    ),
    ("stride_ratio", "high", ACCELERATION): (
        "오버스트라이딩 의심(가속 구간)",
        "보폭이 신장 대비 큽니다. 보폭보다 케이던스(다리 회전수)를 높여 보세요.",
    ),
    ("stride_ratio", "low", MAX_VELOCITY): (
        "보폭 부족(최대 속도 구간)",
        "최대 속도 구간에서 보폭이 좁습니다. 다리를 더 크게 뻗어 보세요.",
    ),
    ("stride_ratio", "high", MAX_VELOCITY): (
        "오버스트라이딩 의심",
        "보폭이 과도합니다. 착지 지점이 무게중심보다 너무 앞서지 않게 하세요.",
    ),
